...
import { BookOpen, Edit3, CalendarCheck, Bell, FileText, ArrowRight, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function SiswaDashboardPage() {
  ...
  const { data: statsData, refresh: refreshStats } = useRealtimeData(
    async () => {
      try {
        setError(null);
        return await api.getSiswaStats();
      } catch (e: any) {
        setError(e.message || 'Gagal memuat statistik siswa.');
        throw e;
      }
    }, 
    45000
  ...
  const { data: notifs } = useRealtimeData(() => api.getNotifications(), 45000);
  ...
  useEffect(() => {
    if (statsData) {
      setStats({
        courses: statsData.active_courses || 0,
        pendingTasks: statsData.pending_tasks || 0,
        attendance: `${statsData.attendance_rate || 0}%`
      });
    }
  }, [statsData]);

  const studentName = isMounted && user?.name ? user.name : 'Siswa';
  const studentInitials = isMounted && user?.name
      ...
      {error && (
        <div className="mb-6 p-6 bg-rose-50 border border-rose-200 rounded-3xl flex items-start gap-4 shadow-xs">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-bold text-rose-900 mb-1">Gagal Memuat Data</h3>
       ...
       <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 mb-8 shadow-xs flex items-center gap-5">
         ...
         <div>
           <h2 className="text-xl font-bold text-slate-900">Halo, {studentName}!</h2>
